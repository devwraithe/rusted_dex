...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if cli.swap_amount < 1000 {
        return Err("swap amount must be at least 1000".into());
    } else {
        // create client
        ...
        let amount_in = cli.swap_amount;
        let minimum_output_amount = 450; // slippage protection

        if amount_in < 1000 {
            return Err("amount to swap must be at least 1000".into());
        } else {
